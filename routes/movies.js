...
router.post("/",middleware.isLoggedIn,(request,respond)=>{
	...
	var nRating = request.body.mRating;
	var nAuthor ={
		id: request.user._id,
...
router.get("/:movieId",(request,respond)=>{
	Movie.findById(request.params.movieId).populate("comments").exec(function(err,foundMovie){
		...
		var sum=0;
		var count=0;
		foundMovie.comments.forEach((comment) => 
		{
			sum += comment.rat;
			count++;
		});
		...
		if (count != 0)
			avg = sum/count;
		respond.render("movies/show",{movie:foundMovie, currentUser:request.user, criticRating: avg.toFixed(1)});
	}
	});
});
// Edit Details
